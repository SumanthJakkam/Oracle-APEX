create or replace package tc_timecards_pkg
   as
   ...
   function get_no_of_leaves(p_start_date    date,
                          p_end_date      date)
   return number;

   function get_week_end(p_date   date)
   return date;

   function get_week_start(p_date  date)
   return date;
   ...
   return number;
   */
   end tc_timecards_pkg;
/

create or replace package body tc_timecards_pkg
   as
   ...
   l_number := (p_end_date - p_start_date) + 1;
   l_date := p_start_date;
   l_leave_days := 0;
   ...
   loop
      ...
      select to_char(l_date, 'DY') into l_day from dual;
 
   if l_day = 'MON' then
      l_leave_days := l_leave_days + 1;
...
null;
   ...
   is
    ...
    begin
       ...
       if l_week_start_month = l_week_end_month then
           dbms_output.put_line('inside l_week_start_month = l_week_end_month '||l_week_start_month ||'='|| l_week_end_month);
		   l_emp_project := emp_project_combi_check(p_emp_id, l_project_id, l_week_start_month, l_week_start_year, l_task_id);
           dbms_output.put_line('l_emp_project '||l_emp_project);
		   if l_emp_project = 0 then 
           dbms_output.put_line('inside l_emp_project = 0 ');
		      select to_char(p_week_start, 'DD') into l_monday from dual;
			  ...
			  sql_stmt := 'insert into tc_time_sheet_details ('|| timesheet_details_id ||',' 
													    ...
													    values(	:l_timesheet_detail_id,
                                                                ...
                                                                :lastupdateby,
                                                                :lastupdatedlogin)';
              EXECUTE IMMEDIATE sql_stmt USING l_timesheet_detail_id, p_emp_id, l_project_id, l_task_id, l_week_start_month, l_week_start_year, p_monday, 
			                                   p_tuesday, p_wednesday, p_thursday, p_friday, p_saturday, p_sunday, sysdate, 1, sysdate, 1, 1;
              commit;	
		   else
           dbms_output.put_line('inside else of l_emp_project = 0 ');
             ...
             sql_stmt2 := 'UPDATE tc_time_sheet_details SET '|| l_day||l_monday ||'= :num ,'
                                                       ...
                                                       WHERE '|| emp_id ||'= :num 
                                                         ...
                                                         and '|| year || '= :num';
			  EXECUTE IMMEDIATE sql_stmt2 USING p_monday, p_tuesday, p_wednesday, p_thursday, p_friday, 
			                                    p_saturday, p_sunday, sysdate, 1, 1, p_emp_id, l_project_id, l_task_id, 
												l_week_start_month, l_week_start_year;
              commit;												
		   end if;
       elsif l_week_start_month <> l_week_end_month then
			 ...
			 if l_new_month_day = 'TUE' then
                ...
                sql_stmt2 := 'UPDATE tc_time_sheet_details SET '|| l_day||l_monday ||'= :num ,'
                                                          ...
                                                          WHERE '|| emp_id ||'= :num  
                                                            ...
                                                            and '|| month || '= :num  
                                                            and '|| year || '= :num';
			    EXECUTE IMMEDIATE sql_stmt2 USING p_monday, sysdate, 1, 1, p_emp_id, l_project_id, l_task_id,
												l_week_start_month, l_week_start_year;
				commit;
                select to_char(l_week_start1, 'DD') into l_tuesday from dual;
                select to_char(l_week_start1 + 1, 'DD') into l_wednesday from dual;
			    ...
			    sql_stmt := 'insert into tc_time_sheet_details ('|| timesheet_details_id ||',' 
													     ...
													     values( :l_timesheet_detail_id,
                                                                 ...
                                                                 :lastupdatedby,
                                                                 :lastupdatelogin)';
                EXECUTE IMMEDIATE sql_stmt USING l_timesheet_detail_id, p_emp_id, l_project_id, l_task_id, l_week_end_month, l_week_end_year, 
			                                   p_tuesday, p_wednesday, p_thursday, p_friday, p_saturday, p_sunday, sysdate, 1, sysdate, 1, 1;
                commit;
			 elsif l_new_month_day = 'WED' then
                dbms_output.put_line('l_new_month_day = WED ');
                ...
                sql_stmt2 := 'UPDATE tc_time_sheet_details SET '|| l_day||l_monday ||'= :num ,'
                                                          ...
                                                          WHERE '|| emp_id ||'= :num 
                                                            ...
                                                            and '|| month || '= :num 
                                                            and '|| year || '= :num';
			    EXECUTE IMMEDIATE sql_stmt2 USING p_monday, p_tuesday, sysdate, 1, 1, p_emp_id, l_project_id, l_task_id,
												l_week_start_month, l_week_start_year;
				commit;
                select to_char(l_week_start1, 'DD') into l_wednesday from dual;
                select to_char(l_week_start1 + 1, 'DD') into l_thursday from dual;				
                ...
                sql_stmt2 := 'UPDATE tc_time_sheet_details SET '|| l_day||l_monday ||'= :num ,'
                                                          ...
                                                          WHERE '|| emp_id ||'= :num 
                                                            ...
                                                            and '|| month || '= :num 
                                                            and '|| year || '= :num';
			    EXECUTE IMMEDIATE sql_stmt2 USING p_monday, p_tuesday, p_wednesday, sysdate, 1, 1, p_emp_id, l_project_id, l_task_id,
												l_week_start_month, l_week_start_year;
				commit;
                select to_char(l_week_start1, 'DD') into l_thursday from dual;				
			    select to_char(l_week_start1 + 1, 'DD') into l_friday from dual;
                ...
                sql_stmt2 := 'UPDATE tc_time_sheet_details SET '|| l_day||l_monday ||'= :num ,'
                                                          ...
                                                          WHERE '|| emp_id ||'= :num 
                                                            ...
                                                            and '|| month || '= :num 
                                                            and '|| year || '= :num';
			    EXECUTE IMMEDIATE sql_stmt2 USING p_monday, p_tuesday, p_wednesday, p_thursday, sysdate, 1, 1, p_emp_id, l_project_id, l_task_id,
												l_week_start_month, l_week_start_year;
				commit;
			    select to_char(l_week_start1, 'DD') into l_friday from dual;
				select to_char(l_week_start1 + 1, 'DD') into l_saturday from dual;
                ...
                sql_stmt2 := 'UPDATE tc_time_sheet_details SET '|| l_day||l_monday ||'= :num ,'
                                                          ...
                                                          WHERE '|| emp_id ||'= :num 
                                                            ...
                                                            and '|| year || '= :num';
			    EXECUTE IMMEDIATE sql_stmt2 USING p_monday, p_tuesday, p_wednesday, p_thursday, p_friday, sysdate, 1, 1,
			                                    p_emp_id, l_project_id, l_task_id, l_week_start_month, l_week_start_year;
				commit;
				select to_char(l_week_start1, 'DD') into l_saturday from dual;
				select to_char(l_week_start1 + 1, 'DD') into l_sunday from dual;
				l_timesheet_detail_id := tc_time_sheet_details_seq.nextval;
                ...
                sql_stmt2 := 'UPDATE tc_time_sheet_details SET '|| l_day||l_monday ||'= :num ,'
                                                          ...
                                                          WHERE '|| emp_id ||'= :num 
                                                            ...
                                                            and '|| year || '= :num';
			    EXECUTE IMMEDIATE sql_stmt2 USING p_monday, p_tuesday, p_wednesday, p_thursday, p_friday, 
			                                    p_saturday, sysdate, 1, 1, p_emp_id, l_project_id, l_task_id,
												l_week_start_month, l_week_start_year;
				commit;
				select to_char(l_week_start1, 'DD') into l_sunday from dual;
				l_timesheet_detail_id := tc_time_sheet_details_seq.nextval;
			    sql_stmt := 'insert into tc_time_sheet_details ('|| timesheet_details_id ||',' 
													     ...
													     values( :l_timesheet_detail_id,
                                                                 ...
                                                                 :lastupdatedby,
                                                                 :lastupdatelogin)';
                EXECUTE IMMEDIATE sql_stmt USING l_timesheet_detail_id, p_emp_id, l_project_id, l_task_id, l_week_end_month, 
                                                 l_week_end_year, p_sunday, sysdate, 1, sysdate, 1, 1;
                commit;			 
			 end if;
	   end if;
